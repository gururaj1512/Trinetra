...
class CrowdAssessment(BaseModel):
    crowd_level: CrowdLevel = Field(..., description="Qualitative crowd density level.")
    estimated_people: int = Field(..., description="Estimated number of people in the crowd.")
    police_required: bool = Field(..., description="Whether police presence is recommended.")
    police_count: int = Field(..., description="Recommended number of police personnel.")
    medical_required: bool = Field(..., description="Whether medical assistance is recommended.")
    medical_staff_count: int = Field(..., description="Recommended number of medical staff.")
    activities: List[str] = Field(..., description="List of observed activities in the crowd.")
    chokepoints_detected: bool = Field(..., description="Whether chokepoints are detected.")
    emergency_access_clear: bool = Field(..., description="Whether emergency access routes are clear.")
    harm_likelihood: str = Field(..., description="Assessment of potential harm likelihood.")
...
def analyze_video(video_path: str, location: str = "Mahakumbh, Prayagraj", context: str = "") -> dict:
    ...
    agent = Agent(
        ...
        system_prompt=(
            "You are a crowd safety expert analyzing Mahakumbh Mela footage. "
            "Be precise, factual, and focus on visible evidence only."
        ),
    )
